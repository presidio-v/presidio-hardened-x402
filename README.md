...
> v0.11.1 — security patch: percent-encoded PII no longer evades the filter. `alice%40example.com` in a resource URL previously passed through unredacted; matching now runs against a bounded percent-decode, with spans mapped back so your own bytes are what you get returned. **Upgrade from any earlier version.**
>
...
| v0.11.1 | **Security patch** — percent-encoded PII no longer evades `PIIFilter` (matching runs against a bounded decode, spans mapped back so caller bytes are preserved) · MPA webhook response body bounded on the plain-httpx path (CWE-400) · NLP install pinned to `en_core_web_lg`, the model Presidio actually loads |
| **v0.11.0** | **CapabilityEnforcer** — opt-in, default-off `capability-grant@1` enforcement stage on the payment path (`PII → wallet → capability → policy → replay → MPA`); reuses `verify_chain` / `check_payment` / `policy_config_from_chain` / `decision_ref`; `StageTiming` per-stage breakdown; byte-identical when unset — **latest feature release** |
