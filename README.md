...
| **v0.5.0** | **OEM embed kit** — rail-agnostic `ScreeningPipeline` core + `bindings/x402` layer (`PaymentProtocolBinding` protocol, hedge against rail fragmentation: ACP/Tempo, AP2) · [SEMVER.md](SEMVER.md) stability guarantees · partner conformance suite (`python -m presidio_x402.conformance`) · CDP/LangChain/CrewAI quickstarts · SBOM in CI · freshness-bound MPA countersignatures (F-8) — **latest release** |
| Unreleased on `main` | MiCA/EU evidence module · `action_ref` derivation primitive · minimum-safe dependency floor updates · GitHub Actions runtime updates |
| v0.6.0 | Multi-tenant key scoping · enterprise-tier remote audit sinks (S3 / Splunk / Datadog) · commissioned human penetration test before paid tiers · SLSA L3 hardened build worker · hard startup-gates for `PRESIDIO_X402_*_KEY` env vars |
